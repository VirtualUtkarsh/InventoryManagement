      ...
      <table className="min-w-full table-auto border-collapse text-sm">
        <thead className="bg-gray-100 text-gray-600 uppercase text-left tracking-wider">
          <tr>
            ...
            <th className="px-4 py-3 min-w-[180px]">Product Name</th>
          ...
          {inventory.map((item) => (
            <tr key={item._id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-2 break-words">
                {item.productName || item.name || item.productId?.productName || `Item ${item.sku}`}
              </td>
              <td className="px-4 py-2 text-blue-600 font-semibold">{item.quantity}</td>
              <td className="px-4 py-2">{item.bin}</td>
              <td className="px-4 py-2 text-gray-500">
                {new Date(item.lastUpdated || item.createdAt).toLocaleString()}
              </td>
            </tr>
          ))}
        </tbody>
